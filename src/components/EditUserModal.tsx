...
const EditUserModal: React.FC<EditUserModalProps> = ({ user, onClose, onUpdate }) => {
  const { updateUserCredentials, updateUserProfile } = useAuthStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.password) {
        await updateUserCredentials(user.id, formData.email, formData.password);
      } else {
        await updateUserCredentials(user.id, formData.email);
      }
